...
{
public:
	olcNoiseMaker(wstring sOutputDevice, SynthModule* synthModule, unsigned int nSampleRate = 44100, unsigned int nChannels = 1, unsigned int nBlocks = 8, unsigned int nBlockSamples = 512)
	{
		this->synthModule = synthModule;
...
public:
	...
	void SetUserFunction(double(*func)(double, SynthModule*))
	{
		m_userFunction = func;
	}
...
private:
	double(*m_userFunction)(double, SynthModule*);
		...
		while (m_bReady)
		{
			...
			for (unsigned int n = 0; n < m_nBlockSamples; n++)
			{
				...
				if (m_userFunction == nullptr)
					...
					nNewSample = (T)(clip(m_userFunction(m_dGlobalTime, synthModule), 1.0) * dMaxSample);
